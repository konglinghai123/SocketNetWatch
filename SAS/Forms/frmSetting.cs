            ...
            try
            {
                ...
                if (openFile.ShowDialog() == DialogResult.OK)
                {
                    ...
                    string strConn = string.Format("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};Extended Properties='Excel 8.0;HDR=Yes;IMEX=1;'", filename);
                     ...
                     OleDbDataAdapter Excel_da = new OleDbDataAdapter("select * from [MedicineInfo$]", con);
                     ...
                     for (int i = 0; i < Excel_dt.Rows.Count;i++ )
                     {
                         ...
                         if (IsIP(Excel_dt.Rows[i][2].ToString())&&IsIntger(Excel_dt.Rows[i][3].ToString()))
                         {
                             DataRow dr = dt.NewRow();
                             dr[0] = Excel_dt.Rows[i][0].ToString();
                             dr[1] = Excel_dt.Rows[i][1].ToString();
                             dr[2] = Excel_dt.Rows[i][2].ToString();
                             dr[3] = Excel_dt.Rows[i][3].ToString();
                             dt.Rows.Add(dr);
                         } 
                         else
                         {
